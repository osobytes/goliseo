...
### Open obligation: the shipped bundle carries no notices

**This is an unmet requirement, recorded here rather than in an issue tracker
because it blocks publication, not development.**
...
by default, and nothing re-adds them: the built bundle currently contains **zero**
occurrences of `@license`, `/*!` or the word `copyright`, and `dist-app/` holds
no notice file. The same is true of the wasm module for the MIT/Apache-2.0 and
Unicode-3.0 crates linked into it.

Before GOLISEO is published to anyone outside this repository, one of the
following must be in place, and this section updated to say which:

- a `THIRD_PARTY_NOTICES.txt` (or equivalent) emitted into `dist-app/` and
  served with the app, carrying the full text of the three.js MIT notice, the
  MIT/Apache-2.0 notices for the wasm-linked crates, and the Unicode-3.0
  notice; **or**
- bundler configuration that preserves legal comments in the emitted JavaScript
  (`build.rollupOptions` / minifier `legalComments`), plus a separate notice
  file for the wasm side, which comment preservation cannot cover.

Apache-2.0 additionally requires that any `NOTICE` file shipped by an upstream
project be reproduced; the dual-licensed crates above are all taken under their
MIT option where one exists, which is the simpler path and should be stated
explicitly in whatever notice file is produced.

### Open discrepancy: crate licence metadata contradicts `LICENSE`

`rust/Cargo.toml` declares `license = "MIT"` in `[workspace.package]`, and all
seven crates inherit it via `license.workspace = true`. The repository's
`LICENSE` is PolyForm Noncommercial 1.0.0, and the opening paragraph of this
file says repository-owned code is under PolyForm "unless a file says
otherwise" — seven `Cargo.toml` files currently say otherwise.

`publish = false` is set workspace-wide, so `cargo publish` refuses and nothing
reaches a registry under those terms. It is still a licence statement in tracked
source that contradicts the licence of record, and resolving it — by correcting
the metadata or by deciding the crates really are MIT — is the repository
owner's call, not an editorial one. The TypeScript packages declare no `license` field at all and so
