//! Differential test of `match_snapshot` against the real Lua implementation
//! (README rule 5.9, `v2/tools/lua_reference/README.md`). This is the
//! required determinism evidence for a hashed snapshot: porting the spec
//! proves the port satisfies written-down assertions, not that two clients
...
//! `metrics_spec.lua` uses, since `sim/match.lua` is not ported and cannot
//! build one) — a scratch `conf.lua`/`main.lua` harness per that README, not
//! committed (scratch dirs are session-local).
//!
...
//! that the Rust port satisfies its own ported spec.
...
fn parse_reference(text: &str) -> (&str, &str) {
    ...
    // hashing this exact string with the Rust FNV-1a-64 port and comparing
    // to the value embedded in the test below.
