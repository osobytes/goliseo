...
fn ai_pass_intercept_flags_a_slow_pass_a_nearby_defender_can_step_onto() {
    ...
    let f = f.expect("the lane is cut");
    assert!(f > 0.0 && f < 1.0, "the fraction lies on the lane");
}

#[test]
fn ai_pass_intercept_a_hard_driven_ball_outruns_the_same_defender() {
    ...
    assert_eq!(f, None, "too fast to collect anywhere on the lane");
}
