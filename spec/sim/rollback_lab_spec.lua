...
t.describe("OMP-2 authoritative-reference rollback laboratory", function()
    t.it("pins the live soccer tape digest without a synthetic combat segment", function()
        t.eq(rollback_lab.tape_digest(determinism_evidence.fixture_tape()), "f0b089d4e24f417e")
    end)
    ...
    t.it("uses one logical result for incremental and synchronous execution", function()
        ...
        t.eq(accounting.input.authoritative_bytes, 4189)
        t.eq(accounting.input.predecessor_anchor_bytes, 0)
        t.eq(accounting.input.effective_frame_bytes, 3083)
        t.eq(accounting.input.input_record_bytes, 6899)
        t.eq(accounting.input.total_bytes, 14171)
        t.eq(accounting.output_bytes, 41616)
        t.eq(accounting.event_bytes, 0)
        t.eq(incremental.metrics.peaks.input_bytes, 14171)
        t.eq(incremental.metrics.peaks.output_bytes, 41634)
