...
//
// Button/axis INDEX -> canonical name follows the Gamepad API's "standard"
// mapping (https://www.w3.org/TR/gamepad/#remapping), which is also what
// `bindings.ts`'s `BUTTON_LABELS` map already assumes (its keys --
// "a"/"b"/"x"/"y"/"leftshoulder"/"dpup"/... -- are exactly the canonical
// names a standard-mapped pad produces here).
//
// One divergence worth flagging: this codebase's control vocabulary
// treats the analog triggers as AXES ("triggerleft"/"triggerright" -- see
// bindings.ts's rule 4 comment: a trigger is never delivered as a button
// edge). The browser's standard mapping reports them as BUTTONS 6/7
// instead, each carrying an analog `.value` alongside `.pressed` -- there
// is no separate triggerleft/triggerright entry in `gamepad.axes` on a
// standard-mapped pad. So `getGamepadAxis("triggerleft"/"triggerright")`
// reads `buttons[6]/[7].value` here, not `axes[...]`, to land on the same
// axis-shaped read `GamepadState` promises regardless of which array the
// browser actually puts it in.
...
// Standard Gamepad Mapping button index -> canonical control-vocabulary
// name. Index 16 ("guide"/home) has no entry in bindings.ts and is
...
// Standard Gamepad Mapping axis index -> canonical control-vocabulary
// name. The trigger axes are handled separately (see the module doc):
// they are not in this table because on a standard-mapped pad they never
// appear in `gamepad.axes` at all.
 ...
 * `RawGamepadEvent` -- an axis read never fires one.
 */
export class BrowserGamepadCapture implements GamepadState {
  ...
  /**
   * @param index Which `navigator.getGamepads()` slot to read -- one slot
   *   is all this game uses.
  ...
  /** True when `button` is currently held. */
  isGamepadDown(button: string): boolean {
    return this._buttonsPressed.get(button) ?? false;
  }

  /**
   * `"triggerleft"`/`"triggerright"` resolve from `buttons[6]/[7].value`
   * (see this module's header); every other axis name resolves from
   * `gamepad.axes` via [`GAMEPAD_AXIS_MAP`].
   */
