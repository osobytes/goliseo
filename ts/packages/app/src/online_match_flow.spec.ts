// The case "routes the lobby's synchronized start into the online match" is
// left `describe.skip`/`it.skip` in `@gc/screens`
// (packages/screens/src/online_match_flow.spec.ts) because `@gc/app`
// depends on `@gc/screens`, never the reverse, so `App` (the module this
// case is actually about) is unreachable from that package. `@gc/app` is
// the correct side of that edge; this file is where the case lives.
//
// # What this case can and cannot prove here
//
// A full real end-to-end version of this case would mount two real
// `App`s, drive a real `OnlineLobby` through a real manual offer/answer
// handshake over a fake star transport, and let the online coordinator (a
// ~3,300-line replicated state machine: admission, assignment, the
// pair-preference protocol, readiness, the countdown/freeze barrier) run
// for real to reach a synchronized start.
//
// None of that machinery is available to reproduce for real from this
// package:
//   - Match manifest/session types are Rust-owned (`gc-netcode`) with no
//     `@gc/wasm` bridge -- confirmed by `app.ts`'s own header and by
//     `online_match_flow.spec.ts` (`@gc/screens`) injecting a
//     `MatchSessionPort` fake even for its own real-`Coordinator` cases.
//   - `@gc/screens`'s `lobby_flow.spec.ts` already builds the one
//     hand-written stand-in for the online coordinator's
//     assignment/preference/readiness/countdown protocol this codebase
//     has -- roughly 1,600 lines, deliberately scoped to that package
//     because reimplementing the coordinator a second time would create a
//     second source of truth for exactly the kind of state two peers must
//     agree on bit for bit (see that file's own header). Duplicating it
//     here, one layer up, would add no protocol-correctness coverage
//     beyond what that file already gives (Rust owns correctness;
//     `crates/gc-netcode/tests` is where that's proven) -- it would only
//     be a second copy of the same fake.
//   - A real two-peer signaling handshake needs `@gc/transport`'s fake
//     star, which is not a declared dependency of `@gc/app` (package.json),
//     and this package may not edit `package.json`.
//
// So this case tests at the boundary `App` actually owns: `OnlinePorts`
// (app.ts). What it proves is real and was, before this case existed,
// entirely unverified: `App.startOnlineMatch` used to be an unconditional
// stub that threw without ever reading the mounted lobby's state (see
// app.ts's header) -- a defect this case exposed and app.ts now fixes for
...
// flow. It does not, and cannot from here, prove the online coordinator's
