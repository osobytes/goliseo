...
// tactics, arenas; Rust-owned, ARCHITECTURE.md §4 rule 6) has no
...
// ("the glue that makes a playable browser build... asset
...
// `gc-data/src/players.rs`/`teams.rs`/`formations.rs`/`tactics.rs`/
// `arenas.rs` -- see that file's header). Re-exported under this package's
// real (non-`test_support`) surface so `browser_main.ts` does not have to
// reach into test infrastructure by a relative path to boot.
