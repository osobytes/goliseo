...
function panel(dl: DrawList, rect: Rect, theme: MatchHudTheme, fill: RGB, border: RGB, alpha?: number): void {
  dl.rect("fill", rect.x, rect.y, rect.w, rect.h, fill, { alpha: alpha ?? 0.94, rx: theme.radius, ry: theme.radius });
...
function speciesMark(dl: DrawList, shape: SpeciesShape, x: number, y: number, color: RGB, size: number): void {
  if (shape === "broad") {
...
function stamina(dl: DrawList, rect: Rect, amount: number, scale: number, theme: MatchHudTheme): void {
  ...
  dl.rect("fill", x, y, w * amount, h, amount <= 0.2 ? theme.colors.amber : theme.colors.cyan, { alpha: 0.95 });
...
function readiness(dl: DrawList, rect: Rect, amount: number, scale: number, theme: MatchHudTheme): void {
  const segments = 5;
  ...
  for (let index = 1; index <= segments; index += 1) {
    ...
    dl.rect("fill", rect.x + 8 * scale + (index - 1) * (width + gap), y, width, Math.max(2, 3 * scale), color, { alpha: 0.9 });
  }
}

/** The product match HUD, drawn from an already-computed model/layout. See file header. */
export function matchHudCommands(model: MatchHudModel, layout: MatchHudLayout, theme: MatchHudTheme, viewport: MatchHudViewport): DrawCommand[] {
  const dl = new DrawList();
  const scale = layout.scale;
  const C = theme.colors;

  dl.text(model.venue, layout.venue.x, layout.venue.y, layout.venue.w, "center", C.text_muted, { fontPx: fontPx(theme, "eyebrow", scale) });
  ...
  dl.rect("fill", layout.scorebug.x + layout.scorebug.w / 2, layout.scorebug.y, layout.scorebug.w / 2, 3 * scale, C.amber);
  dl.text(model.home_name, layout.scorebug.x + 14 * scale, layout.scorebug.y + 17 * scale, 170 * scale, "left", C.text, {
    fontPx: fontPx(theme, "body", scale),
  });
  dl.text(model.away_name, layout.scorebug.x + layout.scorebug.w - 184 * scale, layout.scorebug.y + 17 * scale, 170 * scale, "right", C.text, {
    fontPx: fontPx(theme, "body", scale),
  });
  dl.text(
  ...
  dl.text(model.clock, layout.clock.x, layout.clock.y + 8 * scale, layout.clock.w, "center", C.text, {
    fontPx: fontPx(theme, "body", scale),
  });

  dl.text(model.possession, layout.status.x + 18 * scale, layout.status.y + 2 * scale, layout.status.w - 18 * scale, "center", C.text, {
  ...
  dl.polygon(
    ...
    [diamondX, diamondY - diamondR, diamondX + diamondR, diamondY, diamondX, diamondY + diamondR, diamondX - diamondR, diamondY],
    C.cyan,
  );

  panel(dl, layout.identity, theme, C.panel, C.border);
  dl.rect("fill", layout.identity.x, layout.identity.y, 5 * scale, layout.identity.h, model.species_color, {
    rx: theme.radius,
    ry: theme.radius,
  });
  speciesMark(dl, model.species_shape, layout.identity.x + 27 * scale, layout.identity.y + 24 * scale, model.species_color, 8 * scale);
  dl.text(`${model.player_name} · ${model.player_detail}`, layout.identity.x + 46 * scale, layout.identity.y + 10 * scale, layout.identity.w - 58 * scale, "left", C.text, {
    fontPx: fontPx(theme, "body", scale),
  });
  dl.text("STAMINA", layout.identity.x + 10 * scale, layout.identity.y + 43 * scale, 52 * scale, "left", C.text_muted, {
    fontPx: fontPx(theme, "eyebrow", scale),
  });
  dl.text(
    model.player_state,
  ...
  if (model.equipment_label !== undefined) {
    panel(dl, layout.combat, theme, C.panel, model.feedback_text !== undefined ? C.amber : C.border_soft);
    ...
    dl.text(text, layout.combat.x + 8 * scale, layout.combat.y + 4 * scale, layout.combat.w - 16 * scale, "center", model.feedback_text !== undefined ? C.amber : C.text, {
      fontPx: fontPx(theme, "eyebrow", scale),
    });
  ...
  dl.text(model.plan, layout.plan.x + 8 * scale, layout.plan.y + 15 * scale, layout.plan.w - 16 * scale, "center", C.amber, {
    fontPx: fontPx(theme, "eyebrow", scale),
  });

  if (model.prompt !== undefined) {
    panel(dl, layout.prompt, theme, C.panel_raised, C.cyan);
    dl.text(model.prompt.title, layout.prompt.x + 10 * scale, layout.prompt.y + 8 * scale, layout.prompt.w - 20 * scale, "center", C.cyan, {
      fontPx: fontPx(theme, "eyebrow", scale),
    });
    dl.text(model.prompt.body, layout.prompt.x + 10 * scale, layout.prompt.y + 27 * scale, layout.prompt.w - 20 * scale, "center", C.text, {
      fontPx: fontPx(theme, "body", scale),
    });
  }
  ...
  if (model.announcement_title !== undefined) {
    ...
    dl.text(model.announcement_title, layout.announcement.x + 12 * scale, layout.announcement.y + 24 * scale, layout.announcement.w - 24 * scale, "center", highlight, {
      fontPx: fontPx(theme, "title", scale),
    });
    dl.text(model.announcement_detail ?? "", layout.announcement.x + 12 * scale, layout.announcement.y + 62 * scale, layout.announcement.w - 24 * scale, "center", C.text, {
      fontPx: fontPx(theme, "body", scale),
    });
  }
