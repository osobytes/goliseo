...
> **Status note (2026-08-11).** `scripts/check_rollback.sh` was deleted with the LÖVE tree in
> [#467](https://github.com/osobytes/goliseo/pull/467) and has no replacement yet, so everything
> below that describes a *campaign* run — the native/browser matrix, the stress and soak runs — is
> a record of how the evidence was produced, not a description of a gate that runs today.
> Restoring it is owned by [#472](https://github.com/osobytes/goliseo/issues/472). What *does* run
> today is called out inline where it matters; see "The warning that arrives first" below for the
> retained-storage budgets specifically.
...
Three different numbers describe the margin and they are **not** interchangeable:

- a **synthetic** `(combat_base + combined_delta) * 31` window, the quantity #209 tracked. It now
  lives in `rust/crates/gc-sim/tests/match_snapshot.rs` (it was
  `spec/sim/match_snapshot_spec.lua`'s `snapshot_active_ai_budget` diagnostic, gated by the since-
...
- the rollback lab's **campaign peak** — the high-water mark across a full impaired run, which is
  ...
  same budgets for the four pinned combat load fixtures.
...
**The warning that arrives first.** The 32 KiB margin is still the warning threshold; what applies
it changed. `scripts/check_snapshot_headroom.sh` and the Lua `snapshot_active_ai_budget` marker it
gated were deleted with the LÖVE tree in
[#467](https://github.com/osobytes/goliseo/pull/467), and for a while nothing measured anything
against `Omp2RollbackBudgets` at all — the surviving Rust test compared the authored constants to
...
a below-floor measurement fails too, so a check that quietly stops measuring cannot pass as the most
comfortable possible result. It runs in hundredths of a second under `cargo test --workspace`, which
...
the runner. `memory_growth_ratio` only means anything over a long run, and a seconds-long proxy for
