...
| Product flow | Title → team sheet → match → result completes once in every required environment | First-time player reaches kickoff in ≤90 s; returning path in ≤30 s | #3 scripted flow plus manual stopwatch; record route and result | Compatibility must cover the actual product, not only a blank canvas |
