...
Snapshot version 11 explicitly lists every `MatchState` and `MatchPlayer` field
...
the keeper's post-dive get-up window, both teams' tactic transition windows, the
possession-transition memory, and the current event list. Capture and restore deep-copy all tables, and
...
snapshots are now version 11. Version 9 added the authored formation
...
turnover winner, and seconds elapsed since that turnover). Capture rejects a
...
Combat-capable boundaries use MatchSnapshot version 12. The top-level snapshot
...
cosmetic state are excluded. Version 11 rejects a combat companion, version 12
...
Combat tapes use envelope/identity version 2 and require snapshot version 12.
