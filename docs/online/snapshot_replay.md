...
Snapshot version 10 explicitly lists every `MatchState` and `MatchPlayer` field
...
the keeper's post-dive get-up window, and the current event list. Capture and restore deep-copy all tables, and
...
snapshots are now version 10. Version 9 added the authored formation
identifiers and OutfieldDecision v2's optional run expiry, while changing
decision records to a compact positional encoding. Role and run-drive inputs
remain derived from formation order and serialized match stats rather than
...
under the fixed timestep, and is cleared by the kickoff reset. Soccer-only
...
at `881917e3ba798703`. Version 10 preserves the version-1 soccer tape envelope
and all 7,201 effective input wires, not snapshot byte identity.

Combat-capable boundaries use MatchSnapshot version 11. The top-level snapshot
...
cosmetic state are excluded. Version 10 rejects a combat companion, version 11
...
adds the keeper's `keeper_get_up_timer` scalar to every player record.
Version 11 appends an explicit `combat` record and `GCCS;` schema header after
the version-10 match payload.
...
Combat tapes use envelope/identity version 2 and require snapshot version 11.
...
Snapshot-v10 also has a bounded synthetic replay regression for the goal window
