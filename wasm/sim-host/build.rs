...
//!
//! `ROOTS` itself is the caveat, and it is worth naming: a brand-new top-level
//! Lua directory is neither walked nor watched, so adding one means editing
//! `ROOTS` by hand with no build failure to catch a forgotten edit. That is the
//! same set the old `--preload-file` arguments hardcoded, so it is not new --
//! but it is the one place the stale-manifest risk still lives.
//!
